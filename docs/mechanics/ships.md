...
**Era notes:** Breakwater-pattern sloops carry no hyperdrive as printed — the Admiralty ferries them between systems clamped to fleet tenders, which is why one can "drop in" on a beacon relight and why, once on station, it is never leaving until the tender comes back. The BNS prefix is *Breakwater Naval Ship* — a registry no chart on this side of the dark lists. Maintenance is the one thing the Breakwater yards still do perfectly; this is the rare hull in the era with nothing patched.
